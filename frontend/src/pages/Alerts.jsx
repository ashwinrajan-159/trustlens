...
export default function Alerts() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold text-stone-800">Fraud alerts</h1>
      ...
      {fmr && (
        <Card title="RBI FMR report" action={<Button variant="ghost" onClick={() => setFmr(null)}>Close</Button>}>
          <pre className="overflow-x-auto rounded bg-stone-900 p-3 text-xs text-stone-100">{JSON.stringify(fmr, null, 2)}</pre>
        </Card>
      )}
      ...
      <Card>
        {loading ? <Spinner /> : items.length ? (
          <table className="w-full text-sm">
            <thead className="text-left text-xs uppercase text-stone-400"><tr><th className="py-2">Alert</th><th>Type</th><th>Sev</th><th>Status</th><th>RBI</th><th>SLA</th><th></th></tr></thead>
            <tbody className="divide-y divide-stone-900/10">
              {items.map((a) => (
                <tr key={a.id} className={a.sla_breached ? "bg-red-50" : ""}>
                  ...
                  <td className="text-xs text-stone-400">{a.sla_breached ? <span className="text-red-600">breached</span> : dt(a.sla_deadline)}</td>
                  <td className="space-x-2 text-right text-xs">
                    <Link to={`/app/applications/${a.application_id}`} className="text-brand-700">App</Link>
                    ...
                    {["OPEN", "ACKNOWLEDGED", "ESCALATED"].includes(a.status) && (
                      <>
                        <button className="text-brand-700" disabled={busy === a.id} onClick={() => act(() => api.ackAlert(a.id), a.id)}>Ack</button>
