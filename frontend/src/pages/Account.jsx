    ...
    <div className="max-w-xl space-y-5">
      ...
      <Card title="Profile">
        <div className="space-y-1 text-sm">
          ...
          <div><span className="text-stone-400">Email:</span> {user?.email}</div>
          <div><span className="text-stone-400">Role:</span> <Badge className="bg-brand-100 text-brand-700">{role}</Badge></div>
        </div>
      </Card>
      ...
      <Card title="Multi-factor authentication (TOTP)">
        {!enroll ? (
          ...
          <div className="space-y-3">
            <p className="text-sm text-stone-500">Add this secret to your authenticator app, then enter a code to confirm.</p>
            <div className="rounded bg-stone-900/5 px-3 py-2 font-mono text-sm">{enroll.secret}</div>
            <a href={enroll.provisioning_uri} className="text-xs text-brand-700 break-all">{enroll.provisioning_uri}</a>
      ...
      <Card title="Data privacy (DPDP Act 2023)">
        <p className="mb-3 text-sm text-stone-500">You may withdraw consent to processing of your personal data. This is recorded in the immutable audit trail and triggers the data-erasure workflow.</p>
